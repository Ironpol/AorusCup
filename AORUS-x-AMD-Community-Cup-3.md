# AORUS x AMD - Community Cup #3
## Official Rulebook & Regulations (Counter-Strike 2)

**Organizers :** Centurio, Croissant Strike & Aztec 
**Sponsor :** AORUS, AMD
**Format :** 100% Online - Tier 2 (NON-VRS) 
**Official Website :** https://playaztec.com/ 
**Contact :** contact@playaztec.com  
**Official Discord :** https://discord.com/invite/g2jkVgVArB  

**Official broadcast :**
- https://www.twitch.tv/aorus_emea 
- https://www.twitch.tv/magilacs 

---

# Table of Contents

1. Introduction  
2. General Rules  
3. Tournament Structure  
4. Qualifier Rules  
5. Playoffs Rules  
6. Match Rules (CS2)  
7. Streaming & Media  
8. Prize Pool & Payments  
9. Violations & Penalties  

---

# 1. Introduction

## 1.1 Acceptance of the Rules

AORUS x AMD - Community Cup #3 (hereinafter referred to as "the Tournament") is an official competition organized by Centurio, Croissant Strike & Aztec.

By participating in the Tournament, each player and team confirms that they have read, understood, and accepted all rules stated in this document.

Participation in any official match implies full acceptance of this rulebook.

---

## 1.2 Tournament Officials

The Tournament is organized and administered by Centurio, Croissant Strike & Aztec.

All decisions made by Tournament Administration are final.

---

## 1.3 Official Matches

An "Official Match" refers to any match played within:
- National Qualifiers
- Playoffs

All matches are played via FACEIT unless otherwise stated.

---

# 2. General Rules

## 2.1 Rule Changes

The Tournament Administration reserves the right to modify these rules at any time in order to protect competitive integrity.

Any major changes will be communicated via the official Discord.

---

## 2.2 Communication

Official communication channels:
- Discord
- Email

All private communication with admins is confidential. Public sharing of internal decisions or messages may result in penalties.

---

## 2.3 Player Conduct

All participants must behave respectfully toward:
- Opponents
- Admins
- Viewers
- Staff

Harassment, discrimination, toxicity, or unsportsmanlike behavior may result in penalties up to disqualification.

---

## 2.4 Age Restrictions

Players under 18 years old must be able to provide parental authorization if requested.

---

## 2.5 Accounts & Identity

Players must use their official FACEIT accounts.

Smurfing, account sharing, ban evasion, or fraudulent identity usage may result in immediate disqualification.

---

## 2.6 Rosters & Substitutions

- 5 starting players
- Up to 2 substitutes allowed
- Roster changes allowed before roster lock
- Roster lock occurs on Day 2 of each qualifier
- After roster lock, changes require admin approval

Teams must always respect nationality core rules (see section 4.2).

---

# 3. Tournament Structure

## 3.1 Qualifiers

8 National Qualifiers:

- UK (+Ireland)
- Nordics (Sweden, Denmark, Finland)
- Spain
- Benelux (Belgium, Netherlands, Luxembourg)
- Poland
- Romania
- Turkey
- France (+Switzerland)

Each qualifier:
- 64 slots
- BO1 format
- Check-in: 17:55–18:55 CET
- Start: 19:00 CET

Schedule:

- UK Day 1: 18.03.26
- UK Day 2: 19.03.26
- Nordics Day 1: 19.03.26
- Nordics Day 2: 20.03.26
- Spain Day 1: 20.03.26
- Spain Day 2: 21.03.26
- Benelux Day 1: 21.03.26
- Benelux Day 2: 22.03.26
- Poland Day 1: 22.03.26
- Poland Day 2: 23.03.26
- Romania Day 1: 23.03.26
- Romania Day 2: 24.03.26
- Turkey Day 1: 24.03.26
- Turkey Day 2: 25.03.26
- France Day 1: 25.03.26  
- France Day 2: 26.03.26 

---

## 3.2 Playoffs

- 8 Qualified Teams
- Single Elimination
- All matches BO3
- Dates: 27/03 & 28/03

---

# 4. Qualifier Rules

## 4.1 Platform

All qualifiers and playoffs are played on FACEIT.

FACEIT Anti-Cheat is mandatory.

---

## 4.2 Nationality Rule (Core Requirement)

Each team must have a minimum of 3 players of the qualifier's nationality.

This core of 3 players must always be maintained, including when using substitutes.

Failure to respect nationality rules may result in disqualification.

---

## 4.3 Match Format (Qualifiers)

- BO1
- MR12
- Overtime: MR3 (6 rounds total per OT)

---

# 5. Playoffs Rules

## 5.1 Format

- Single Elimination
- All matches BO3
- MR12
- Overtime MR3 until winner

---

## 5.2 Map Veto Procedure

### BO1 Veto Format

The veto process for BO1 matches is conducted as follows:

1. Team A bans 1 map  
2. Team B bans 1 map  
3. Team A bans 1 map  
4. Team B bans 1 map  
5. Team A bans 1 map  
6. Team B bans 1 map  

The remaining map will be played.

The team designated as Team A / Team B is determined by the FACEIT platform or by tournament administration if required.

Sides will be determined by standard FACEIT knife round unless otherwise specified.

### BO3 Veto Format

The veto process for BO3 matches in the Playoffs is conducted as follows:

1. Team A bans 1 map  
2. Team B bans 1 map  
3. Team A picks Map 1  
4. Team B picks Map 2  
5. Team A bans 1 map  
6. Team B bans 1 map  

The remaining map will be played as the Decider (Map 3).

---

### Side Selection

- On Map 1, Team B chooses the starting side.
- On Map 2, Team A chooses the starting side.
- On the Decider map, the starting side will be determined by a knife round unless otherwise specified by tournament administration.

---

### Team Designation

The designation of Team A and Team B is determined by:

- FACEIT seeding system, OR
- Tournament administration decision if required.

Administration decisions are final.

## 5.3 Webcams (Mandatory Playoffs)

Webcams are mandatory for official matches.

Failure to provide a working webcam when requested may result in:
- Map loss
- Match loss
- Disqualification

Specific webcam instructions will be provided via Discord.

---

## 5.4 Coaching

Coaches are allowed.

However, tournament administration may intervene in case of abuse or unfair advantage.

---

# 6. Match Rules (CS2)

## 6.1 Game Version

All matches must be played on the latest official version of Counter-Strike 2.

---

## 6.2 Map Pool

- Dust2  
- Mirage  
- Nuke  
- Overpass  
- Ancient  
- Inferno  
- Anubis  

---

## 6.3 Match Settings

- MR12 (first to 13 rounds)
- Overtime MR3
- Repeated OTs until a winner is determined

---

## 6.4 Pauses

Per map:

- 3 Tactical Pauses (30 seconds each)
- Overtime: +1 Tactical Pause per side
- 1 Technical Pause (5 minutes maximum)

Important:
If a team uses its own voice server (Discord/Teamspeak), no technical pause will be granted for voice-related issues.

Abuse of pauses may result in penalties.

---

## 6.5 Disconnects & Issues

Teams must immediately notify admins in case of:

- Player disconnect
- Server crash
- Technical issue affecting competitive integrity

Admins reserve the right to:
- Restore the round
- Replay the round
- Award the round
- Issue a forfeit

---

# 7. Streaming & Media

## 7.1 Official Broadcast

Official streams:
- https://www.twitch.tv/croissantstrike
- https://www.twitch.tv/croissantstrike2

The Tournament reserves the right to broadcast any match.

---

## 7.2 Player Streaming

Players are allowed to stream their matches with a mandatory 120-second delay.

---

## 7.3 Broadcast Rights

For co-streaming or rebroadcast authorization, contact:
Ironpol via Discord.

Unauthorized commercial rebroadcasts are prohibited.

---

# 8. Prize Pool & Payments

## 8.1 Total Prize Pool

€10,000

---

## 8.2 Distribution

- 1st Place: 50% (€5,000)
- 2nd Place: 25% (€2,500)
- 3rd Place: 12.5% (€1,250)
- 4th Place: 12.5% (€1,250)

---

## 8.3 Payment Delay

Prize payments will be processed within 3 months after tournament completion.

---

## 8.4 Payment Recipient

Payments will be made to:
- Team Captain
OR
- Organization

The Tournament is not responsible for internal distribution among players.

---

# 9. Violations & Penalties

## 9.1 Cheating

Any form of cheating, including but not limited to:

- Third-party software
- Exploits
- Account sharing
- Ban evasion

Results in immediate disqualification and possible ban from future events.

---

## 9.2 Match-Fixing & Betting

Any involvement in:

- Match manipulation
- Betting on tournament matches
- Sharing inside information for betting purposes

Results in immediate disqualification.

---

## 9.3 No-Show & Forfeit

Failure to join a match on time may result in:

- Map forfeit
- Match forfeit
- Disqualification

---

## 9.4 Brand Damage

Public attempts to damage the reputation of:
- The Tournament
- Organizers
- Sponsors

May result in disqualification and exclusion from future events.

---

# Final Authority

All decisions made by Tournament Administration are final.

Participation implies full acceptance of all rules.
